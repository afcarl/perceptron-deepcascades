...
The Deep Cascades \cite{deepcascades} algorithm is a cascade of a rich family of functions structured as a decision tree, with the  node question and leaf predictors being picked from this rich family. The algorithm exhibits data dependent guarantees that allow for use of a rich family of functions. The paper talks about using SVMs as the rich set of functions and elaborates on how the deep cascades algorithm is adapted for use with SVMs.
...
The paper on Deep cascades presents data dependent generalization guarantees for cascades with leaf predictors chosen from a hypothesis set $H_k$ and node question functions selected from hypothesis sets $Q_j$. They present the following guarantee on the generalization error in the simplest form:
\begin{equation}
...
leaf hypothesis sets $H_k$ are decision surfaces defined by polynomial kernels.
The hypothesis $h_k \in H_k$ is learned via the SVM algorithm with a polynomial
kernel degree $\delta_k$ on subsample $S_k$ . The node question hypothesis set $Q_k$ is
the set of indicator functions which indicate if a point is within a distance $c$ from the hyperplane such that a $\mu_k|S_k|$ points lie within that distance $c$ from the hyperplane. 
\\Hence a search is done over all depths $[1 \cdots L]$, fraction of points $\mu_k \in \mathcal{M}$ and degree of kernels $\delta_k \in \mathcal{G}$ such that the resulting cascade minimizes the generalization error. We also see that the rademacher complexities from (\ref{dcbound}) have been bounded by the VC dimension of the respective SVM ($d_{f,j},d_{f,k}$) and that a factor $\gamma$ has been introduced to scale the hinge loss. Thus the DeepCascadeSVM only tries to get the minimal upper bound  in terms of the VC dimension rather than trying to minimize the rademacher complexities.
...
\\In the case of the kernelized perceptron we actually need to learn a hyperplane in the kernel hilbert space for which we first need to identify the dual formulation. This is done by observing that $w$ can be expressed as  linear combination of the training samples. The equation being $w = \sum_{i=1}^{m} \alpha_i y_i x_i$. Then we can see that adjusting with $y_ix_i$ in the primal form is the same as adjusting the $\alpha_i$ by 1. Now we see that the predicted $\widehat{y}_i$ is given using $sgn(w\cdot x_i)$. Writing this with the dual we see that:
\par
{\footnotesize
\begin{equation}
\begin{aligned}
...
\Function {DeepCascadePerceptron}{$S,L,\mathcal{M},\mathcal{G},\gamma$}	
	\For{$l \in [1 \cdots L],(\mu_k)_{1 \leq k \leq l} \subseteq \mathcal{M},(\delta_k)_{1 \leq k \leq l} \subseteq \mathcal{G}$}
		...
		\For{$ k = 1$ to $l$}
			...
			\State{$threshold \gets get\_distance(S_k,h_k,\mu_k,poly\_kernel(\delta_k))$}	\Comment{Gets the distance of $\mu_k$ fraction of closest point from hyperplane}
			\State{$q_k \gets 1_{dist(x,h_k) \leq threshold}$}
			\State{$S_{k+1} \gets q_{k}^{-1}(1) \cap S_k$}
		\EndFor
		\State{$f(\cdot) \gets \sum_{k=1}^{l-1} \left( \prod_{j=1}^{k-1} q_j \right) \bar{q_k}h_k + \left( \prod_{j=1}^{l} q_j \right) h_l$}
		\State{$\mathcal{F} \gets \mathcal{F} \cup \{ f \} $}
...
\\The algorithm essentially consists of training perceptrons $h_k$ at each level (total of $l$ levels) with the incoming points using polynomial kernels of dimension $\delta_k$. Once the perceptron at level $k$ is trained on $S_k$, the distances to all points are measured as $w_k.x = \sum_{i=1}^{|S_k|} \alpha_i y_i K(x_i,x_t)$ (ignoring the factor of $\lVert w_k \rVert$ as we just want to get the $\mu_k$ fraction of closest points). With the distances calculated we decide on a threshold as the distance from the hyperplane such that $\mu_k|S_k|$ points lie within that distance. The node question is then: if a given point lies within this threshold from the trained hyperplane. This process is then repeated for each level.
...
\\The VC dimension of a perceptron accepting samples of dimension $n$ and working with a polynomial kernel of dimension $\delta$ is $\binom{n+\delta}{\delta}$. This can be seen by the definition of polynomial kernel: ($K(x,y) = (1+x \cdot y)^\delta$) and using multinomial theorem.
\\The $\gamma$ is selected by doing a n-fold cross validation to select the best $\gamma$ from a set of values.

\section{Experiments}
We performed some preliminary experiments on datasets from UCI machine learning repository \cite{Lichman:2013}.
...
\hline
Dataset & Number of examples & Number of features & col1 & DeepCascadePerceptron & Cascade Depth\\
\hline
\end{tabular}
\caption{Test error for DeepCascadePerceptron}
\end{table}
